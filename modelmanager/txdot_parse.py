...
from sklearn import (metrics, model_selection, linear_model, preprocessing,
                     ensemble, neighbors, decomposition)
...
def print_total(df, value):
    ...
    mask = df.select_dtypes([
        np.object
    ]).apply(lambda x: x.str.contains(r"%s" % value, case=False, na=False))
...
This includes converting '0' for "missing" to 'np.nan' to prevent the modelling algorithms from evaluating it as a real value. This also improves runtime performance, as a proper null value is evaluated quicker than an integer representation (numpy knows not to evaluate np.nan, but can't know in advance whether an 'int' is '0' and therefore spends extra time to evaluate it)
    ...
    if (1):
        ...
        for cat in data.columns:
            data[cat] = data[cat].apply(
                lambda x: x if type(x) != str else re.sub(
                    r"%s" % re_missing, re_replace, x, flags=re.IGNORECASE))
    ...
    data['speed_limit'].replace(0, np.nan,
                                inplace=True)  # speed_limit - np.nan is faster
    data['speed_limit'].replace(-1, np.nan,
                                inplace=True)  # speed_limit - np.nan is faster
...
def clean_data(datafile, source='txdot', verbose=0):
    ...
    colgrps = {
        ...
        'intersection':
        ['street_name', 'intersecting_street_name', 'intersection_related'],
    }
    ...
    def process_data_punctuation(df):
        ...
        for category in list(featdef[featdef.type == 'str'].index):
            df[category] = df[category].str.replace(regexpunct,
                                                    '_').str.lower()
    ...
    for cat in data.columns:
        ...
        data[cat] = data[cat].apply(lambda x: x if type(x) != str else re.sub(
            '[,\s()]+', '_', x).lower())
...
def preprocess_data(datafile, source='txdot', verbose=0):
    ...
    featdef = add_feature(featdef, 'crash_time_30m', {
        'type': '24h',
        'regtype': 'categorical',
        'origin': 'crash_datetime'
    })
    # </crash_time>
    return (data, featdef)
...
def preproc_add_bin_categories(data, featdef, verbose=0):
    ...
    if (1):
        ...
        data[bin_category[1]] = data[bin_category[0]].str.replace(
            '[,\s()-]+', '_').str.lower()
        ...
        data[bin_category[1]].replace(
            ['non_incapacitating_injury', 'possible_injury', 'not_injured'],
            1,
            inplace=True)
        data[bin_category[1]].replace(['incapacitating_injury', 'killed'],
                                      0,
                                      inplace=True)
        data[bin_category[1]].replace(['unknown'], np.nan, inplace=True)
        # track new feature
        featattr = dict(featdef.ix[bin_category[0]])
        featdef = add_feature(featdef, bin_category[1], {
            'type': 'int',
            'regtype': 'bin_cat',
            'target': featattr['target']
        })
        ...
        data[bin_category] = data['intersection_related'].str.replace(
            regexpunct, '_').str.lower()
        ...
        data[bin_category] = data['light_condition'].str.replace(
            regexpunct, '_').str.lower()
        ...
        # 1 :
        bin_true = [
            'one_motor_vehicle_going_straight', 'angle_both_going_straight',
...
def impute_mph(data, verbose=0):
    ...
    colgrps = {
        ...
        'inter_mph': [
            'speed_limit', 'crash_year', 'street_name',
    ...
    df_inter_nomph = df_inter[(df_inter.speed_limit.isnull()
                               )][colgrps['intersection']].drop_duplicates()
    if (verbose):
        print("intersections without speed_limit:\nout: %s" %
              (df_inter_nomph.shape[0]))
    ...
    for i, ser in df_inter_nomph.iterrows():
        ...
        if (verbose3):
            print("%s | %s | %s" %
                  (i, ser.street_name, ser.intersecting_street_name))
        ...
        #
        dftmp = data[(data['street_name'] == ser['street_name'])
                     & (data['intersecting_street_name'] ==
                        ser['intersecting_street_name'])].copy()
        if (verbose3):
            print("before:")
        ...
        for jq in dftmp.index:
            ...
            if (np.isnan(data.ix[jq].speed_limit)
                ):  # | (data.ix[jq].speed_limit == -1) ):
...
def get_data_quality_report(data):
    ...
    for row in df_quality_rpt.index:
        ...
        tmpser = (pd.Series({
            ...
            'cardinality': data[row].unique().size
        }))
        tmpser.name = row
    ...
    # 1th+ is mean,std, etc
    colorder = [
        df_quality_rpt.columns[0]
    ] + newvals.columns.values.tolist() + df_quality_rpt.columns[1:].tolist()
    colorder = [
        'count', '%missing', 'cardinality', 'mean', 'std', 'min', '25%', '50%',
        '75%', 'max', 'freq', 'top'
    ]  # ,'unique'
...
if (__name__ == '__main__'):
    ...
    # save to file
    with open('data_quality_report.html', 'w') as outfile:
        df_quality_rpt.fillna('').to_html(outfile)
...
AttributeError: 'AxesSubplot' object has no attribute 'rowNum'
    ...
    if (show_data_vis_model):
        ...
        datapt = data.pivot_table(values=[
            'crash_death_count', 'crash_incapacitating_injury_count',
            'crash_non-incapacitating_injury_count'
        ],
                                  index=['speed_limit', 'crash_time'])
        print(datapt)
